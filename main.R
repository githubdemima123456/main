...
setwd("E:/BaiduSyncdisk/PANZIWEN/code")
cov.sel.high<-function(T=NULL, Y=NULL, X=NULL, betahat=TRUE, parallel=FALSE, 
                       Simulate=TRUE, N=NULL, Setting=1, rep=1, Models=c("Linear", "Nonlinear", "Binary")
  ...
  for(i in 1:Rep){
    ...
    if(length(unique(dat[,ycol]))>2){
        tried<-try(arules::discretize(dat[,ycol], method="interval"), silent=TRUE)
        if(class(tried)=="factor"){
          dat[,ycol]<-tried
        }else{
          print(tried)
        ...
        dat[,ycol]<-factor(dat[,ycol])
      }
      if(length(numcol)>0){
        if(length(numcol)==1){
          tried<-try(arules::discretize(dat[,numcol],method="interval"),silent=TRUE)
          if(class(tried)=="factor"){
            dat[,numcol]<- tried
          }else{ print(tried)
    ...
    res1<-mmhc(lt1, blacklist = blacklistT, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
    ##Subset X.T
    XT<-res1$nodes$T$mb
    #print(XT)
    ...
    if(is.data.frame(lt2)==FALSE){
      ...
      res2<-mmhc(lt2, blacklist = blacklistQ, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
      Q1<-res2$nodes$Y$mb
    }
    ...
    if(is.data.frame(lt3)==FALSE){
      ...
      res3<-mmhc(lt3, blacklist = blacklistQ, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
      Q0<-res3$nodes$Y$mb
    }
    ##Subset Q=Q.1UQ.0
    ...
    res4<-mmhc(lt4, blacklist = blacklistY, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
    res5<-mmhc(lt5, blacklist = blacklistY, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
    ##Subset X.1
    X1<-res4$nodes$Y$mb
    ##Subset X.0
    X0<-res5$nodes$Y$mb
    ...
    if(is.data.frame(lt6)==FALSE){
      ...
      res6<-mmhc(lt6, blacklist = blacklist1, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
      Z1<-res6$nodes$T$mb
    }
    ##Subset Z.0
    if(is.data.frame(lt7)==FALSE){
      ...
      res7<-mmhc(lt7, blacklist = blacklist0, restrict.args = list(alpha = alpha), maximize.args = list(score = mmhc_score))
      Z0<-res7$nodes$T$mb
    }
    ##Subset Z=Z.1UZ.0
    ...
    if(betahat==TRUE){
      ...
      f1<-as.formula(paste0("T~", paste0(paste0(colnames(datbeta)[covarcol]), collapse= "+")))
      ...
      if(length(XT)==0){
        ...
        f1<-as.formula(paste0("T~", paste0(paste0(XT), collapse= "+")))
        ps<-glm(f1,family=binomial,data=datbeta)$fitted
      ...
      betahatQse<-sqrt((var(datbeta[which(datbeta[,Tcol]==1),ycol])/length(which(datbeta[,Tcol]==1))+var(datbeta[which(datbeta[,Tcol]==0),ycol]))/length(which(datbeta[,Tcol]==0)))
        ...
        f1<-as.formula(paste0("T~", paste0(paste0(Q), collapse= "+")))
        ps<-glm(f1,family=binomial,data=datbeta)$fitted
      ...
      betahatXYse<-sqrt((var(datbeta[which(datbeta[,Tcol]==1),ycol])/length(which(datbeta[,Tcol]==1))+var(datbeta[which(datbeta[,Tcol]==0),ycol]))/length(which(datbeta[,Tcol]==0)))
        ...
        f1<-as.formula(paste0("T~", paste0(paste0(XY), collapse= "+")))
        ps<-glm(f1,family=binomial,data=datbeta)$fitted
      ...
      betahatZse<-sqrt((var(datbeta[which(datbeta[,Tcol]==1),ycol])/length(which(datbeta[,Tcol]==1))+var(datbeta[which(datbeta[,Tcol]==0),ycol]))/length(which(datbeta[,Tcol]==0)))
        ...
        f1<-as.formula(paste0("T~", paste0(paste0(Z), collapse= "+")))
        ps<-glm(f1,family=binomial,data=datbeta)$fitted
      ...
      betahatXDse<-sqrt((var(datbeta[which(datbeta[,Tcol]==1),ycol])/length(which(datbeta[,Tcol]==1))+var(datbeta[which(datbeta[,Tcol]==0),ycol]))/length(which(datbeta[,Tcol]==0)))
        ...
        f1<-as.formula(paste0("T~", paste0(paste0(XD), collapse= "+")))
        ps<-glm(f1,family=binomial,data=datbeta)$fitted
    ...
    reslist[[i]]<-list(X.T=XT,Q.0=Q0,Q.1=Q1,Q=Q,X.0=X0,X.1=X1,X.Y=XY,Z.0=Z0,Z.1=Z1,Z=Z,X.TY=XD,cardinalities=cards, est_psm=betahats_psm, se_psm=betahatsse_psm, est_tmle=betahats_tmle, se_tmle=betahatsse_tmle, N=N, Setting=Setting, rep=Rep, Models=Models, alpha=alpha, mmhc_score=mmhc_score, varnames=varnames)
  l<-list(reslist=reslist)
  invisible(return(l[[1]][[1]]))
}
...
confounder_selection <- function(){
  data <- read.csv("E:/BaiduSyncdisk/PANZIWEN/code/24months_98genes.csv")
  genes <- c("KLK5","CREB3L3","IDH1","SLC12A5","TNFSF14","FOXS1","NLRP7","HTR5A","LILRB2","POU4F1","SSTR4","NPTX2")
  outcome <- as.numeric(data$Group)
  ...
  for(gene in genes){
    treatment <- as.factor(data[[gene]])
    covariates <- data[, !(names(data) %in% c(gene, "Group"))]
    for (col_name in colnames(covariates)) {
      covariates[[col_name]] <- as.factor(covariates[[col_name]])
    }
    result <- cov.sel.high(T = treatment, Y = outcome, X = covariates, 
                           betahat = TRUE, parallel = FALSE, 
                           Simulate = FALSE, alpha = 1,mmhc_score="bic")
    results_df <- rbind(results_df, data.frame(
      Gene = gene,
      X_T = paste(result$X.T, collapse=", "),
      Q_0 = paste(result$Q.0, collapse=", "),
      Q_1 = paste(result$Q.1, collapse=", "),
      Q = paste(result$Q, collapse=", "),
      X_0 = paste(result$X.0, collapse=", "),
      X_1 = paste(result$X.1, collapse=", "),
      X_Y = paste(result$X.Y, collapse=", "),
      Z_0 = paste(result$Z.0, collapse=", "),
      Z_1 = paste(result$Z.1, collapse=", "),
      Z = paste(result$Z, collapse=", "),
      X_TY = paste(result$X.TY, collapse=", "),
      cardinalities = paste(result$cardinalities, collapse=", "),
      est_psm = paste(result$est_psm, collapse=", "),
      se_psm = paste(result$se_psm, collapse=", "),
      est_tmle = paste(result$est_tmle, collapse=", "),
      se_tmle = paste(result$se_tmle, collapse=", "),
      N = paste(result$N, collapse=", ")
    ))
  }
  write.csv(results_df, "E:/BaiduSyncdisk/PANZIWEN/code/mmhc.csv", row.names = FALSE)
}
